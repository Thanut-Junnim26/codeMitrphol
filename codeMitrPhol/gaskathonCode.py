...
def main():
    ...
    st.sidebar.info(
        ...
        - Contact us : <CEO เจเจ> 
        - Contact us : <CEO กัตเบ>🔞
        - road to 500,000 Bath 🏅
        """
    )
    st.title("🌎Greenhouse Glass Track By รวมmitr🪐")
    st.markdown('''แผนที่ฉบับสามารถดูปริมาณmethane 
                ตามรูปภาพถ่ายดาวเทียม ซึ่งจะอัพโหลดทุกๆ 5 วันสามารถดูใหม่ได้อีกครั้ง
                โปรด!!!!รอจนถึงวันนั้น🔨''')

    st.markdown(
        '''แผนที่ฉบับนี้จัดทำโดยกลุ่ม QuakeGas Sentinel ซึ่งเป็นกลุ่มที่ร่วมเข้าแข่งขัน gaskathon ห้ามเผยแพร่ก่อนได้รับอนุญาตโดยเด็ดขาด🪓''')
    ...
    # Map.to_streamlit()
    Map.add_legend(title="Methane",
                   legend_dict=act_legend, position='bottomright', draggable=False, style=style)
    Map.to_streamlit(height=500, width=700)


# 4/1AfJohXlNQ8jCV_qnivvNw1I_7YwVkf6kpjKjpcKBespHlaFH7P5fz4xIvXg
# ee.Authenticate()
ee.Initialize()
main()
# line_chart()
